...
public class DistinctElements {
    ...
    //public int calculatePosition(int x, int position){
        //int position = x &
    //}
    ...
    public int calculateHashValue(int x){
        ...
        for(int i = 0; i < 32; i++){
            int hashValuePositioni = Integer.bitCount(x & A[i]) % 2;
            System.out.println(hashValuePositioni);

            if (hashValuePositioni == 1)  {
                result |= (hashValuePositioni << i);
    ...
    public static void main(String[] args) {
        ...
        System.out.println(de.calculateHashValue(2147483647));
